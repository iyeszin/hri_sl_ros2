# inference with the actions client
import rclpy
...
def main(args=None):
    ...
    # Create instances of the webcam publisher and the action client
    webcam_publisher = WebcamPublisher()
    action_client = CountdownClient()

    # Create a thread for running the webcam publisher
    webcam_thread = threading.Thread(target=webcam_publisher.capture_frames)

    # Start the webcam publisher thread
    webcam_thread.start()
    ...
    # Wait for the webcam publisher thread to finish
    webcam_thread.join()
