...
from rh8d_msgs.msg import LoneJoint, LoneSensor, LoneMainBoard, AllMainBoards, JointsSensorsCombined,\
    ClearHWError, JointListSetStiffness, JointListSetSpeedPos, SetShutdownCond, SensorUserCommand
...
class RH8DHWNode(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ('light_mode', False),
                ('hand_polarity', 'L_'),
                ('hand_baudrate', 1000000),
                ('sensor_baudrate', 1000000),
        ...
        try:
            self.hand_baudrate = self.get_parameter('hand_baudrate').value
            self.get_logger().info("Baudrate is : %s" % self.hand_baudrate)
        except KeyError:
            self.get_logger().info("Could not find hand baudrate parameter, setting to default (1000000)")
            pass

        try:
            self.sensor_baudrate = self.get_parameter('sensor_baudrate').value
            self.get_logger().info("Baudrate is : %s" % self.sensor_baudrate)
        except KeyError:
            self.get_logger().info("Could not find sensor baudrate parameter, setting to default (1000000)")
            pass

        try:
            self.hand_port = self.get_parameter('hand_port').value
            self.get_logger().info("Hand port name is: %s" % self.hand_port)
        except KeyError:
            self.get_logger().info("Could not find hand port parameter, setting to default port (/dev/ttyUSB0)")
            pass

        try:
            self.sensor_port = self.get_parameter('sensor_port').value
            self.get_logger().info("Sensor port name is: %s" % self.sensor_port)
        except KeyError:
            self.get_logger().info("Could not find sensor port parameter, setting to default port (/dev/ttyUSB1)")
            pass
        ...
        try:
            self.number_sensors = int(self.get_parameter('number_sensors').value)
            self.get_logger().info("Sensor number is : %s" % self.number_sensors)
        except KeyError:
            self.get_logger().info("Could not find number_sensors parameter, setting to default (5)")
            pass
        ...
        try:
            ...
            self.get_logger().info("Joint mapping imported, main board mapping is: %s" % self.main_board_mapping)
        except KeyError:
        ...
        self.dxl_stiffness_perm_ID = []
        self.dxl_stiffness_perm_val = []

        # Storage lists for Joints, main boards for ROS Msg
        self.joints = []
        self.main_boards = []
        self.sensors = [Sensor() for _ in range(self.number_sensors)]
        ...
        # rate variables
        self.rate_raw_sensor_msg = self.create_rate(10)

        ################################################################################################################
        # setup Serial Interface to Hand
        self.portHandler = PortHandler(self.hand_port)
        ...
        # setup serial interface to the Sensors
        try:
            self.sensor_read = serial.Serial(self.sensor_port, self.sensor_baudrate, timeout=1.5, write_timeout=1)

        except serial.SerialException:
            self.get_logger().error("Could not open serial port %s" % self.sensor_port)
            quit()

        self.get_logger().info('Serial connection to RH8D tactile sensors established!')
        ...
        try:
            self.portHandler.setBaudRate(self.hand_baudrate)
        ...
        self.lone_sensor_list = [LoneSensor() for _ in range(self.number_sensors)]
        self.joints_sensors_combined = JointsSensorsCombined()
        ...
        self.setStiffness_subscriber = self.create_subscription(JointListSetStiffness, self.polarity + 'setStiffness', self.setStiffnessCallback, 10, callback_group=self.callback_group1)
        ...
        self.sensor_write_sleep = self.create_rate(10)
        self.sensor_usr_cmd = self.create_subscription(SensorUserCommand, self.polarity + 'sensor_usr_cmd', self.sensor_usr_cmd_callback, 10)
        self.sensor_usr_cmd

        self.joint_publisher_ = self.create_publisher(JointsSensorsCombined, self.polarity + 'JointsSensorsCombined', 10)
    ...
    def initJoints(self):
        ...
        for joint in self.joints:
            ...
            dxl_add_param_result = self.groupSyncRead.addParam(joint.id)
            if not dxl_add_param_result:
                self.get_logger().error("[ID:%03d] groupSyncRead add param failed" % joint.id)
                quit()
    ...
    def initMainBoards(self):
        for i in self.main_board_ID_list:
            ...
            dxl_add_param_result = self.groupSyncReadMainBoard.addParam(i)
            if not dxl_add_param_result:
                self.get_logger().error("[ID:%03d] groupSyncReadMainBoard add param failed" % i)
                quit()
            ...
            try:
                ...
                for i, sensor in enumerate(self.sensors):
                    try:
                        ...
                        self.get_logger().error('ValueError in sensor reading')
                    except IndexError:
                        self.get_logger().error('IndexError in sensor reading')
                        self.get_logger().info("%s" % i)

                return timestamp
            except IndexError:
                self.get_logger().error('IndexError with time stamp')
    ...
    def fillJointMsg(self):
        ...
        for message, sensor in zip(self.lone_sensor_list, filtered_sensors_):
        ...
        self.joints_sensors_combined.header.stamp = self.get_clock().now().to_msg()
        self.joints_sensors_combined.numjoints = len(self.dxl_read_ID_list)
        self.joints_sensors_combined.numsensors = self.number_sensors
        self.joints_sensors_combined.joints = self.lone_joint_list
        self.joints_sensors_combined.sensors = self.lone_sensor_list
    ...
    def sensor_usr_cmd_callback(self, data):
        ...
        if data.set_frequency:
            period_ms_ = (1 / data.frequency) * 1000  # User sets frequency, converting to ms to send the message to the sensors
        ...
        if data.raw_string:  # If the user wants to send a custom command
            ...
            self.rate_raw_sensor_msg.sleep()
    ...
    def clearErrorCallback(self, msg):
        ...
        if id_ != 'None':
            ...
            if not dxl_add_param_result:
                self.get_logger().error("groupSyncWriteClearError add param failed on joint %s" % msg.name)
                return
        else:
    ...
    def setStiffnessCallback(self, msg):
        ...
        self.dxl_stiffness_perm_ID.clear()
        self.dxl_stiffness_perm_val.clear()
        ...
        for joint in msg.joints:
            ...
            if id_ != 'None':
                ...
                self.dxl_stiffness_perm_ID.append(id_)
                ...
                if 1 <= stiffness <= 9:
                    ...
                    self.dxl_stiffness_perm_val.append(perm_param)
        ...
        for index, id_ in enumerate(self.dxl_write_ID_stiffness):
            ...
            if not dxl_add_param_result:
                self.get_logger().error("[ID:%d] groupSyncWriteStiffness add param failed, abort wrtiting" % id_)
                return

        # set permissions to the hardware
        for index, id_ in enumerate(self.dxl_stiffness_perm_ID):
            dxl_add_param_result = self.groupSyncWritePermission.addParam(id_, self.dxl_stiffness_perm_val[index])
            if not dxl_add_param_result:
                self.get_logger().error("[ID:%d] groupSyncWritePermission add param failed, abort writing" % id_)
    ...
    def setSpeedPosCallback(self, msg):
        ...
        for index, id_ in enumerate(self.dxl_write_ID_pos_speed):
            ...
            if not dxl_add_param_result:
                self.get_logger().error("[ID:%d] groupSyncWriteSpeedPos add param failed, abort writing" % id_)
                return
    ...
    def mainLoopCallback(self):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            self.get_logger().warn("Communication not successful in SyncRead : %s" % self.packetHandler.getTxRxResult(dxl_comm_result))
        ...
        self.joint_publisher_.publish(self.joints_sensors_combined)
        ...
        if time.time_ns() / 1000000 > self.cycle_end:
            ...
            if not self.light_mode:
                ...
                if dxl_comm_result != COMM_SUCCESS:
                    self.get_logger().warn("Communication not successful in groupSyncReadMainBoard : %s" % self.packetHandler.getTxRxResult(dxl_comm_result))
...
class Joint:
    ...
    def printJointInfo(self):
        ...
        self.node.get_logger().info("Geartrain model %d" % self.gear_train_model)
